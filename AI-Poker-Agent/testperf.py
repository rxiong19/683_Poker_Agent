...
import time
import game
from alt_player import AltPlayer
import sys
...
def testperf(agent_name1, agent1, agent_name2, agent2):
    ...
    config = setup_config(
        max_round=max_round, initial_stack=initial_stack, small_blind_amount=smallblind_amount)

    # Register players
    config.register_player(name=agent_name1, algorithm=AltPlayer())
    config.register_player(name=agent_name2, algorithm=TestPlayer())
...
def parse_arguments():
    ...
    parser.add_argument('-n1', '--agent_name1',
                        help="Name of agent 1", default="Your agent", type=str)
    parser.add_argument('-a1', '--agent1', help="Agent 1", default=AltPlayer())
    parser.add_argument('-n2', '--agent_name2',
                        help="Name of agent 2", default="Your agent", type=str)
    parser.add_argument('-a2', '--agent2', help="Agent 2",
                        default=RaisedPlayer())
